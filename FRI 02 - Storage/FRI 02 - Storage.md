...
* **Persistent Volumes (PV):** 💽 A PV is a piece of actual storage infrastructure that has been provisioned for the cluster to use. It exists independently of any single pod's lifecycle.
...
For our lab, we will be using the default storage class provided by the cluster (e.g., `vultr-block-storage`).
...
Create a file named `1-pvc.yaml` (or use the one provided):
```yaml
apiVersion: v1
...
Apply the claim using `envsubst` to inject your namespace:
```bash
envsubst < 1-pvc.yaml | kubectl apply -f -
```

⚠️ **Troubleshooting:** If you see an error saying `failed to download openapi`, you can bypass it by adding `--validate=false`:
`envsubst < 1-pvc.yaml | kubectl apply -f - --validate=false`
...
kubectl get pv
```
*Notice the output! You will see a dynamically generated PV with a long, random name (e.g., `pvc-1a2b3c4d...`). Look at the `CLAIM` column—you will see it is explicitly mapped to your namespace/claim.*

💡 **What is happening here?** We asked for `1Gi` using the defined policy (`get pvc`). Kubernetes saw this request, talked directly to the storage provider, created a physical disk, registered it as a Persistent Volume (`get pv`), and tied the two together. 
...
Create a file named `2-stateful-pod.yaml` (or use the one provided):
```yaml
apiVersion: v1
kind: Pod
metadata:
...
```bash
envsubst < 2-stateful-pod.yaml | kubectl apply -f -
```
...
```bash
kubectl exec data-server -n $NS -- sh -c "echo '<h2>Success! This data is PERSISTENT!</h2>' > /usr/share/nginx/html/index.html"
```

Verify the data is there:
```bash
kubectl exec data-server -n $NS -- cat /usr/share/nginx/html/index.html
```
...
```bash
kubectl delete pod data-server -n $NS
```
...
Wait for it to run (`kubectl get pods -n $NS`), then check the data again:
```bash
kubectl exec data-server -n $NS -- cat /usr/share/nginx/html/index.html
```
...
Apply the updated PVC using `envsubst`:
```bash
envsubst < 1-pvc.yaml | kubectl apply -f -
```
...
```bash
kubectl get pvc my-data-claim -n $NS
kubectl get pv
```
*(Note: It may take a minute for the underlying storage to process the expansion. You might need to restart the pod to trigger the filesystem resize).*
...
envsubst < 2-stateful-pod.yaml | kubectl delete -f -
envsubst < 1-pvc.yaml | kubectl delete -f -
